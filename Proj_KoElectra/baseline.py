...
class BaseInfo:
    def __init__(self):
        ...
        self.category_extraction_model_path = '/saved_model/category_extraction/'
        self.polarity_classification_model_path = '/saved_model/polarity_classification/'

        self.test_category_extraction_model_path = '/saved_model/category_extraction/saved_model_example.pt'
        self.test_polarity_classification_model_path = '/saved_model/polarity_classification/saved_model_example.pt'

        self.train_data_path = '/data/train_data/nikluge-sa-2022-train.jsonl'
        self.dev_data_path = '/data/dev_data/nikluge-sa-2022-dev.jsonl'
        self.test_data_path = '/data/test_data/nikluge-sa-2022-test.jsonl'
...
class DataParser:
    ...
    def tokenize_and_align_lables(self, tokenizer, form, annotations, max_len):
        ...
        for pair in self.entity_property_pair:
            ...
            if pd.isna(form):
                break
            tokenized_data = tokenizer(form, pair, embedding='max_length', max_length=max_len, truncation=True)
    ...
    def get_dataset(self, raw_data, tokenizer, max_len):
        ...
        self.polarity_dataset = TensorDataset(
            torch.tensor(polarity_input_ids_list), torch.tensor(polarity_attention_mask_list), torch.tensor(polarity_token_labels_list)
            )
...
class Trainer:
    ...
    def evaluation(self, y_true, y_pred, label_len):
        ...
        self.y_true = list(map(int, y_true))
        self.y_pred = list(map(int, y_pred))
    ...
    def train_sentiment_analysis(self):
        ...
        tokenizer = AutoTokenizer.from_pretrained(bi.base_model)
        ...
        print('loading model')
        entity_property_model = RoBertaBaseClassifier(len(bi.tf_name_to_id), len(tokenizer))
        entity_property_model.to(bi.device)

        polarity_model = RoBertaBaseClassifier(len(bi.polarity_name_to_id), len(tokenizer))
        ...
        if FULL_ENDING:
            ...
            entity_property_optimizer_grouped_parameters = [
                {'params': [p for n, p in entityOproperty_param_optimiizer if not any(nd in n for nd in no_decay)], 'weight_decay': 0.01},
                {'params': [p for n, p in entityOproperty_param_optimiizer if any(nd in n for nd in no_decay)], 'weight_decay': 0.0}
                ]
        else:
            entity_property_param_optimiizer = list(entity_property_model.classifier.named_parameters())
            entity_property_optimizer_grouped_parameters = [{'params': [p for n, p in entity_property_param_optimiizer]}]
...
class EvaluateModel:
    ...
    def test_sentiment_analysis(self):
        ...
        test_data = dh.jsonlload(bi.test_data_path)
        ...
        model = RoBertaBaseClassifier(len(bi.tf_id_to_name), len(tokenizer))
        model.load_state_dict(torch.load(bi.test_category_extraction_model_path, map_location=bi.device))
        model.to(bi.device)
        model.eval()

        polarity_model = RoBertaBaseClassifier(len(bi.polarity_id_to_name), len(tokenizer))
        polarity_model.load_state_dict(
            torch.load(bi.test_polarity_classification_model_path, map_location=bi.device))
