...
g.add_argument("--ensemble-models", type=str, default="", help="Comma-separated list of model names for ensembling")
...
def main(args):
    ...
    if args.ensemble_models:
        ...
        ensemble_models = args.ensemble_models.split(',') if args.ensemble_models else [args.model_path]
        models = []
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        for model in models:
            trainer = Trainer(model, targs, compute_metrics=None)
            predictions = trainer.predict(encoded_vds).predictions
            
            if ensemble_predictions is None:
                ensemble_predictions = torch.sigmoid(torch.Tensor(predictions))
            else:
                ensemble_predictions += torch.sigmoid(torch.Tensor(predictions))
        ...
        result = multi_label_metrics(predictions=ensemble_predictions, labels=p.label_ids)
        return result
    ...
    for model in models:
        trainer = Trainer(
            ...
            tokenizer=tokenizer,
            compute_metrics=compute_metrics if model == models[-1] else None
        )
        trainer.train()
